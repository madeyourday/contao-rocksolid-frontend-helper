...
class FrontendHooks
{
    ...
    public function __construct(
        ...
        private readonly array $backendModules = [],
    ) {
    }
    ...
    public function parseFrontendTemplateHook($content, $template)
    {
        ...
        if (\in_array('infos', $permissions, true)) {
            try {
                if (
            ...
            if (\in_array('tpl_editor', $permissions, true)) {
        ...
        if (preg_match('(<[a-z0-9]+\\s(?>"[^"]*"|\'[^\']*\'|[^>"\'])+)is', $content, $matches)) {
            ...
            if (preg_match('(^(.*\\sclass="[^"]*)rsfh-article-([0-9]+)-([0-9a-f]*)(.*)$)is', $matches[0], $matches2)) {
                ...
                if (\in_array('articles', $permissions, true)) {
                ...
                if (\in_array('contents', $permissions, true)) {
                    $data['container'] = 'tl_article:'.$matches2[2];
                    $data['table'] = 'tl_content';

                    System::loadLanguageFile('tl_content');
                    $data['links']['pastenew'] = [
                        'url' => static::getBackendURL('article', 'tl_content', $matches2[2], 'create', ['mode' => 2, 'pid' => $matches2[2]]),
                    ...
                    if ($lastContentElement && $lastContentElement->id) {
                        $data['links']['pastebottom'] = [
                            'url' => static::getBackendURL('article', 'tl_content', $matches2[2], 'create', ['mode' => 1, 'pid' => $lastContentElement->id]),
    ...
    public function outputFrontendTemplateHook($content, $template)
    {
        ...
        if (\in_array('pages', $permissions, true)) {
        ...
        if (\in_array('feModules', $permissions, true)) {
        ...
        if (\in_array('rstAssistant', $permissions, true) && $assistantId = static::getThemeAssistantStylesheet()) {
            $data['links']['assistant'] = [
     ...
     */
    public function parseTemplateHook($template): void
    {
        ...
        if (\in_array('beModules', $permissions, true)) {
            if (
                str_starts_with($template->getName(), 'com_')
                && str_starts_with((string) $template->id, 'c')
            ) {
                $template->class .= ' rsfh-comment-'.substr((string) $template->id, 1);
            }

            if (
                str_starts_with($template->getName(), 'rsmm_')
                && !empty($template->id)
            ) {
    ...
    public function parseWidgetHook($content, $widget)
    {
        ...
        if (\in_array('contents', $permissions, true) && is_numeric($widget->id) && is_numeric($widget->pid)) {
            ...
            $data['links']['pastenew'] = [
                'url' => static::getBackendURL('form', 'tl_form_field', $widget->pid, 'create', ['mode' => 1, 'pid' => $widget->id]),
        ...
        if (
            \in_array('infos', $permissions, true)
            && 'form_rsce_plain' !== $widget->template
            && 'form_rs_columns_plain' !== $widget->template
        ) {
     ...
     */
    public function parseArticlesHook($template, $row, $module): void
    {
        if (!($permissions = static::checkLogin()) || !\in_array('beModules', $permissions, true)) {
            return;
        }
     ...
     */
    public static function getArticleHook($row): void
    {
        ...
        $cssId[1] = trim($cssId[1].' rsfh-article-'.$row->id.'-'.bin2hex((string) $row->inColumn ?: ''));
        ...
        if (\in_array('beModules', $permissions, true)) {
            foreach ($this->backendModules as $do => $config) {
                ...
                if (
                    ...
                    && \in_array($row->type, $config['fe_modules'], true)
                ) {
                    $id = null;
                    if (!empty($config['column']) && !empty($row->{$config['column']})) {
                        $id = $row->{$config['column']};
                        if (isset($config['column_type']) && 'serialized' === $config['column_type']) {
    ...
    public function getContentElementHook($row, $content, $element)
    {
        ...
        if (\in_array('contents', $permissions, true)) {
            ...
            if ($row->ptable) {
                foreach ($GLOBALS['BE_MOD'] as $category) {
                    foreach ($category as $moduleName => $module) {
                        if (
                            !empty($module['tables'])
                            && \in_array($row->ptable, $module['tables'], true)
                            && \in_array('tl_content', $module['tables'], true)
                        ) {
                            $do = $moduleName;
                            break 2;
                        }
            ...
            if (
                ...
                && !\in_array($row->type, (array) FrontendHelperUser::getInstance()->elements, true)
            ) {
                $editAllowed = false;
            }
            ...
            $data['links']['pastenew'] = [
                'url' => static::getBackendURL($do, 'tl_content', $row->pid, 'create', ['mode' => 1, 'pid' => $row->id]),
        ...
        if ('module' === $row->type && $row->module) {
            $moduleRow = ModuleModel::findById($row->module);
            if ($moduleRow) {
                $content = $this->getFrontendModuleHook($moduleRow, $content);
            }
        } elseif (\in_array('beModules', $permissions, true)) {
            foreach ($this->backendModules as $do => $config) {
                if (!empty($config['do'])) {
                    $do = $config['do'];
                }
                if (
                    ...
                    && \in_array($row->type, $config['content_elements'], true)
                ) {
                    $id = null;
                    if (!empty($config['ce_column']) && !empty($row->{$config['ce_column']})) {
                        $id = $row->{$config['ce_column']};
                        if (isset($config['ce_column_type']) && 'serialized' === $config['ce_column_type']) {
                            $id = StringUtil::deserialize($id, true);
                            if (empty($id[1])) {
                                $id = $id[0];
                            } else {
     ...
     *
     * @return array|bool false if the user isn't logged in otherwise the permissions array
     */
    public static function checkLogin()
    {
        ...
        if ($User->isAdmin) {
            return ['feModules', 'beModules', 'pages', 'articles', 'contents', 'infos', 'rstAssistant', 'tpl_editor'];
        }
     ...
     * @param bool  $withParentTable use the edit label from the parent table if possible
     *
    ...
    {
        if (str_starts_with((string) $data['templatePath'], 'templates/')) {
            $data['templateURL'] = static::getBackendURL('tpl_editor', null, $data['templatePath'], 'source');

            System::loadLanguageFile('tl_files');
            $data['templateLabel'] = \sprintf(\is_array($GLOBALS['TL_LANG']['tl_files']['source']) ? $GLOBALS['TL_LANG']['tl_files']['source'][1] : $GLOBALS['TL_LANG']['tl_files']['source'], basename((string) $data['templatePath']));
        } else {
            $data['templateURL'] = static::getBackendURL(
                'tpl_editor',
                null,
                null,
                null,
     ...
     * @return string
     */
    protected static function getBackendURL($do, $table, $id, $act = 'edit', array $params = [])
    {
        $addParams = [];
    ...
    protected static function insertData($content, $data)
    {
        if (
            ...
            if ('html' === $matches[1] && -1 !== strpos($content, '<body')) {
            ...
            if (preg_match('(\\sdata-frontend-helper="([^"]*)")is', $matches[0], $matches2)) {
                ...
                if (isset($oldData['links'], $data['links'])) {
    ...
    private static function route(string $routeName): string
    {
        ...
        if ($previewScript && substr((string) $url, 0, \strlen($previewScript) + 1) === "$previewScript/") {
            $url = substr((string) $url, \strlen($previewScript));
        }
